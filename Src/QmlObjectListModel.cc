...
Q_LOGGING_CATEGORY(QmlObjectListModelLog, "QmlObjectListModelLog")
...
QmlObjectListModel::~QmlObjectListModel() {
    if (_resetModelNestingCount > 0) {
        qCWarning(QmlObjectListModelLog)
            << "QmlObjectListModel destroyed with unbalanced nesting of "
               "begin/endResetModel calls - _resetModelNestingCount:"
...
bool QmlObjectListModel::insertRows(int position, int rows,
    ...
    if (position < 0 || position > _objectList.count() + 1) {
        qCWarning(QmlObjectListModelLog) << "Invalid position - position:count"
                                         << position << _objectList.count() << this;
    }
...
bool QmlObjectListModel::removeRows(int position, int rows,
    ...
    if (position < 0 || position >= _objectList.count()) {
        ...
        qCWarning(QmlObjectListModelLog)
            << "Invalid rows - position:rows:count" << position << rows
...
QObject *QmlObjectListModel::removeAt(int i) {
    ...
    if (removedObject) {
        ...
        if (_objectList[i]->metaObject()->indexOfSignal(
                QMetaObject::normalizedSignature("dirtyChanged(bool)")
                    .constData()) != -1) {
...
void QmlObjectListModel::insert(int i, QObject *object) {
    if (i < 0 || i > _objectList.count()) {
        qCWarning(QmlObjectListModelLog)
            << "Invalid index - index:count" << i << _objectList.count() << this;
    }
    if (object) {
...
void QmlObjectListModel::beginResetModel() {
    if (_resetModelNestingCount == 0) {
        qCDebug(QmlObjectListModelLog)
            << "First call to begindResetModel - calling "
    ...
    qCDebug(QmlObjectListModelLog)
        << "_resetModelNestingCount:" << _resetModelNestingCount << this;
}

void QmlObjectListModel::endResetModel() {
    if (_resetModelNestingCount == 0) {
        qCWarning(QmlObjectListModelLog) << "QmlObjectListModel::endResetModel "
                                            "called without prior beginResetModel";
        return;
    }
    _resetModelNestingCount--;
    qCDebug(QmlObjectListModelLog)
        << "_resetModelNestingCount:" << _resetModelNestingCount << this;
    if (_resetModelNestingCount == 0) {
        qCDebug(QmlObjectListModelLog) << "Last call to endResetModel - calling "
